...
if [ ! -f "/etc/ssl/certs/xdmod-selfsigned.crt" ]; then
    ...
    # Создаем конфигурацию с SAN для IP (совместимо с Chrome)
...
mysql -u root -p${MYSQL_ROOT_PASSWORD} -e "UPDATE modw.person SET organization_id = 1 WHERE id = (SELECT person_id FROM moddb.Users WHERE username = 'admin');" 2>/dev/null || true
...
# Обновление ACL конфигурации
echo "Обновление ACL конфигурации..."
/usr/share/xdmod/bin/acl-config 2>/dev/null || true
