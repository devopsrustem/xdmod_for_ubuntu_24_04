...
# Проверка доступности SLURM sacct
if [ -f "/opt/slurm/bin/sacct" ]; then
    echo "Проверка совместимости sacct..."
    if ldd /opt/slurm/bin/sacct 2>&1 | grep -q "not found"; then
        echo "ВНИМАНИЕ: Обнаружены проблемы с зависимостями sacct:"
        ldd /opt/slurm/bin/sacct 2>&1 | grep "not found"
    else
        echo "sacct совместим с текущей системой"
        # Обновление конфигурации XDMoD
        if [ -f "/etc/xdmod/portal_settings.ini" ]; then
            sed -i 's|sacct = "sacct"|sacct = "/opt/slurm/bin/sacct"|g' /etc/xdmod/portal_settings.ini
            echo "Путь к sacct обновлен в конфигурации XDMoD"
        fi
    fi
else
...
# Импорт данных SLURM каждые 15 минут
*/15 * * * * root /usr/share/xdmod/bin/xdmod-shredder -r tcluster01 -f slurm -d /opt/xdmod-data/slurm-export >> /var/log/xdmod/shredder.log 2>&1
5,20,35,50 * * * * root /usr/share/xdmod/bin/xdmod-ingestor >> /var/log/xdmod/ingestor.log 2>&1
